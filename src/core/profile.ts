...
import { generateEmbedding, EMBEDDING_DIM } from './embedding.js';
...
export function buildProfileFromTags(tags: string[]): InterestProfile {
  const existing = loadProfile();
  return {
    vector: existing?.vector ?? new Array<number>(EMBEDDING_DIM * 2).fill(0),
    tags,
...
/**
 * Rebuild the 768-dim matching vector by concatenating two embeddings:
 *
 *   [0..383]  = embed(human profile: tags + summary + intro)
 *   [384..767] = embed(agent memory summary, ≤256 tokens)
 *
 * Each half is independently embedded via all-MiniLM-L6-v2 (384-dim).
 * The full vector is transmitted ONLY in Paillier-encrypted form.
 */
export async function rebuildVector(): Promise<void> {
  const profile = loadProfile();
  if (!profile) return;

  const profileParts: string[] = [];
  if (profile.tags.length > 0) {
    profileParts.push(`Interests: ${profile.tags.join(', ')}`);
  }
  if (profile.summary) {
    profileParts.push(profile.summary);
  }
  if (profile.intro) {
    profileParts.push(profile.intro);
  }
  const profileText = profileParts.join('\n');

  const ctx = loadAgentContext();
  const summaryText = ctx.memorySummary || '';

  const profileVec = profileText.trim()
    ? await generateEmbedding(profileText)
    : new Array<number>(EMBEDDING_DIM).fill(0);

  const memoryVec = summaryText.trim()
    ? await generateEmbedding(summaryText)
    : new Array<number>(EMBEDDING_DIM).fill(0);

  profile.vector = [...profileVec, ...memoryVec];
